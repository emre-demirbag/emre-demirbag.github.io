...
<style>
  ...
  .card {
    ...
    border-radius: 18px; /* FIX: was invalid token */
    padding: 16px;
    background: white;
  ...
  /* FIX: Definitions column behaves on-screen, no “random scrollbar/gutter” */
  .defsCard{
    align-self: start;
    position: sticky;
    top: 14px;
    max-height: calc(100vh - 28px); /* was 520px -> caused unnecessary inner scroll */
    overflow: auto;
  ...
  /* Touch devices (iPad/iPhone): sticky + overflow can look bad -> use normal page flow */
