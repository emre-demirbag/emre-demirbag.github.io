...
<style>
  ...
  /* FIX: keep headings sane inside the custom UI (theme-safe) */
  .wrap h2{
    font-size: 28px;
    line-height: 1.18;
    margin: 6px 0 12px;
  ...
  .grid {
    ...
    gap: 18px;
    align-items: start; /* FIX: prevent uneven stretching */
  }
  .grid > * { min-width: 0; }

  /* FIX: iPad portrait should still show 2 columns; stack only on smaller screens */
  ...
  .card {
    ...
    color: #0b0b0b; /* FIX: protect against theme text color */
  }
  ...
  .termbar .dots { display:flex; gap:6px; flex: 0 0 auto; }
  .dot { width:10px; height:10px; border-radius:999px; background: rgba(160,255,180,.25); }
  .termbar .title { font-weight: 600; letter-spacing:.2px; flex: 1 1 auto; min-width: 0; }
  #asciiHint { flex: 0 0 auto; opacity: .9; white-space: nowrap; }
  ...
  table { width: 100%; border-collapse: collapse; color: #0b0b0b; } /* FIX: theme-safe */
  th, td { padding: 10px; border-bottom: 1px solid rgba(0,0,0,.10); vertical-align: top; }
  ...
  .modalBody { padding: 14px; color: #0b0b0b; } /* FIX: theme-safe */
  ...
  /* FIX: keep right column sticky without inner scrolling (prevents Safari scroll weirdness) */
  .defsCard{
    align-self: start;
    position: sticky;
    top: 14px;
